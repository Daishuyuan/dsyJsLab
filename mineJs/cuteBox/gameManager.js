function PainterMatrix(id, row, col, size) {
    ...
    this.canvas.width = parseInt(this.col * size + (this.col - 1) * this.space);
    this.canvas.height = parseInt(this.row * size + (this.row - 1) * this.space);
    this.context = this.canvas.getContext("2d");
    this.matrix = [];
    this.special = [];
    this.colorTable = [];
    this.noSpeeking = false;
    for (let i = 0; i < row; ++i) {
        this.matrix.push(this.fillArray(this.void));
        this.colorTable.push(this.fillArray(this.backColor));
    }

    this.step = function (x, y, action, color) {
        let code = NaN;
        this.drawBlock(x, y, this.backColor);
        x += action.x;
        y += action.y;
        if (x >= 0 && x < this.row && y >= 0 && y < this.col) {
            this.drawBlock(x, y, color);
            code = this.matrix[x][y];
        }
        return [x, y, code];
    }
    ...
    this.repainter = function () {
        for (let i = 0; i < this.row; ++i) {
            for (let j = 0; j < this.col; ++j) {
                this.drawBlock(i, j, this.colorTable[i][j]);
            }
        }
    }
    ...
    this.drawBlock = function (x, y, color) {
        ...
        this.context.fillRect(y * (this.range + this.space), x * (this.range + this.space), this.range, this.range);
    }

    this.getSpecial = function (num) {
        let array = [];
        for (let elem of this.special) {
            if (elem.num == num) {
                array.push(elem);
            }
        }
    ...
    this.mouseConfigure = function (num, color, callback, multi) {
        function tailCal() {
        ...
        this.canvas.onmousedown = function (e) {
            if (e.buttons === 1) {
                let y = parseInt(e.layerX / (self.range + self.space));
                let x = parseInt(e.layerY / (self.range + self.space));
                if (x >= 0 && x < self.row && y >= 0 && y < self.col && self.matrix[x][y] == self.void) {
                    self.matrix[x][y] = num;
